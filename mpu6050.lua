...
local MPU6050_ADDRESS_AD0_LOW     =   0x68 -- address pin low (GND), default for InvenSense evaluation board
local MPU6050_ADDRESS_AD0_HIGH    =   0x69 -- address pin high (VCC)
...
local MPU6050_RA_USER_CTRL      =   0x6A
local MPU6050_RA_PWR_MGMT_1     =   0x6B
local MPU6050_RA_PWR_MGMT_2     =   0x6C
local MPU6050_RA_WHO_AM_I       =   0x75
CountMpu6050 = 0
Sstate={x=0.00,y=0.00,z=0.00}
--器件ID检测
local function MPU6050_check()
    i2c.send(i2cid, MPU6050_ADDRESS_AD0_LOW, MPU6050_RA_WHO_AM_I)--读器件地址
    sys.wait(50)
    local revData = i2c.recv(i2cid, MPU6050_ADDRESS_AD0_LOW, 1)
    if revData:byte() ~= nil then
        i2cslaveaddr = MPU6050_ADDRESS_AD0_LOW
    else
        i2c.send(i2cid, MPU6050_ADDRESS_AD0_HIGH, MPU6050_RA_WHO_AM_I)--读器件地址
        sys.wait(50)
        local revData = i2c.recv(i2cid, MPU6050_ADDRESS_AD0_HIGH, 1)
        if revData:byte() ~= nil then
            i2cslaveaddr = MPU6050_ADDRESS_AD0_HIGH
        else
            log.info("i2c", "Can't find device")
            return 1
        end
    end
    i2c.send(i2cid, i2cslaveaddr, MPU6050_RA_WHO_AM_I)--读器件地址
    sys.wait(50)
...
--获取温度的原始数据
local function MPU6050_get_temp_raw()
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_TEMP_OUT_H)--获取的地址
    local buffer = i2c.recv(i2cid, i2cslaveaddr, 2)--获取2字节
    local _, temp = pack.unpack(buffer, ">h")
    return temp or 0
end
--获取加速度计的原始数据
local function MPU6050_get_accel_raw()
    local accel={x=nil,y=nil,z=nil}
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_ACCEL_XOUT_H)--获取的地址
    local x = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,accel.x = pack.unpack(x,">h")
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_ACCEL_YOUT_H)--获取的地址
    local y = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,accel.y = pack.unpack(y,">h")
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_ACCEL_ZOUT_H)--获取的地址
    local z = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,accel.z = pack.unpack(z,">h")
    return accel or 0
end
--获取陀螺仪的原始数据
local function MPU6050_get_gyro_raw()
    local gyro={x=nil,y=nil,z=nil}
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_GYRO_XOUT_H)--获取的地址
    local x = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,gyro.x = pack.unpack(x,">h")
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_GYRO_YOUT_H)--获取的地址
    local y = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,gyro.y = pack.unpack(y,">h")
    i2c.send(i2cid, i2cslaveaddr,MPU6050_RA_GYRO_ZOUT_H)--获取的地址
    local z = i2c.recv(i2cid, i2cslaveaddr, 2)--获取6字节
    _,gyro.z = pack.unpack(z,">h")
    return gyro or 0
end
--获取温度的原始数据
local function MPU6050_get_temp()
    local temp=nil
    local tmp = MPU6050_get_temp_raw()
...
--获取加速度计的数据，单位: mg
local function MPU6050_get_accel()
    local accel={x=nil,y=nil,z=nil}
    local tmp = MPU6050_get_accel_raw()
    accel.x = tmp.x*1000/MPU6050_ACCEL_SEN
    accel.y = tmp.y*1000/MPU6050_ACCEL_SEN
    accel.z = tmp.z*1000/MPU6050_ACCEL_SEN
    return accel
end
--获取陀螺仪的数据，单位: deg / 10s
local function MPU6050_get_gyro()
    local gyro={x=nil,y=nil,z=nil}
    local tmp = MPU6050_get_gyro_raw()
    gyro.x = tmp.x*100/MPU6050_GYRO_SEN
    gyro.y = tmp.y*100/MPU6050_GYRO_SEN
    gyro.z = tmp.z*100/MPU6050_GYRO_SEN
    return gyro
end
local function MPU6050()
    sys.wait(4000)
    log.info(">>>>>>>>>>>>>>get accel<<<<<<<<<<<<<<<")
    ...
    if MPU6050_check()~= 0 then
        return
    end
    MPU6050_init()
    while true do
        Test1 = MPU6050_get_accel()
        if CountMpu6050< 200 then
            Sstate.x = Sstate.x + Test1.x/1000
            Sstate.y = Sstate.y + Test1.y/1000
            Sstate.z = Sstate.z + Test1.z/1000
            CountMpu6050 = CountMpu6050 + 1
        end
        if CountMpu6050 == 200 then
            Sstate.x = Sstate.x/CountMpu6050
            Sstate.y = Sstate.y/CountMpu6050
            Sstate.z = Sstate.z/CountMpu6050
            CountMpu6050 = CountMpu6050 + 1
        end
        if CountMpu6050 > 200 then
            displacementAlarm.DisplacementAlarm(Test1,Sstate)
        end
        sys.wait(10)
    end
end
sys.taskInit(MPU6050)
