    ...
    function()
        ...
        while true do
            ...
            if socket.isReady() then
                ...
                --创建一个MQTT客户端
                local mqttClient = mqtt.client(imei,240,"admin","keson-123",nil,{qos=0,retain=0,topic=
                ...
                if mqttClient:connect("47.94.80.3",1883,"tcp") then
                    retryConnectCnt = 0
                    ready = true
                    --订阅主题
                    if mqttClient:subscribe({["/quake/iot/hebei"]=0,["gas/check"]=1}) then
                        mqttOutMsg.init()
                        --循环处理接收和发送的数据
                        while true do
